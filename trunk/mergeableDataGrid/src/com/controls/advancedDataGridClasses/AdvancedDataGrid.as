...
/*************************************************************************
*
* Tree Events, Styles and Metadata
*
*************************************************************************/
...
*
*  @eventType com.controls.advancedDataGridClasses.AdvancedDataGridEvent.ITEM_CLOSE
*/
[Event(name="itemClose", type="com.controls.advancedDataGridClasses.AdvancedDataGridEvent")]

/**
*  Dispatched when a branch of the navigation tree is opened or expanded.
*
*  @eventType com.controls.advancedDataGridClasses.AdvancedDataGridEvent.ITEM_OPEN
*/
    [Event(name="itemOpen", type="com.controls.advancedDataGridClasses.AdvancedDataGridEvent")]

/**
*  Dispatched when a tree branch open or close operation is initiated.
*
*  @eventType com.controls.advancedDataGridClasses.AdvancedDataGridEvent.ITEM_OPENING
*/
[Event(name="itemOpening", type="com.controls.advancedDataGridClasses.AdvancedDataGridEvent")]
...
*
*  @eventType com.controls.advancedDataGridClasses.AdvancedDataGridEvent.HEADER_DRAG_OUTSIDE
*/
[Event(name="headerDragOutside", type="com.controls.advancedDataGridClasses.AdvancedDataGridEvent")]
...
*
*  @eventType com.controls.advancedDataGridClasses.AdvancedDataGridEvent.HEADER_DROP_OUTSIDE
*/
[Event(name="headerDropOutside", type="com.controls.advancedDataGridClasses.AdvancedDataGridEvent")]
//--------------------------------------
//  Styles
//--------------------------------------
 ...
 *  @see com.controls.advancedDataGridClasses.AdvancedDataGridEvent
...
public class AdvancedDataGrid extends AdvancedDataGridBaseEx
{
     ...
     *
     *  Use in conjunction with the <code>highlightedIndex</code> property
    ...
    public function set groupedColumns(value:Array):void
    {
        ...
        {
            if(_groupedColumns)
                for ( var i:int = 0; i < _groupedColumns.length; i++)
    ...
    mx_internal function getRendererDescription(itemData:Object,c:AdvancedDataGridColumn, forDragProxy:Boolean = false):AdvancedDataGridRendererDescription
    {
        ...
        for (var i:int = 0; i < rendererProviders.length; i++)
        {
    ...
    override protected function commitProperties():void
    {
        if (dataProviderChanged)
        {
            ...
            if (hierarchicalCollectionInstance)
            {
                ...
                hierarchicalCollectionInstance.addEventListener(CollectionEvent.COLLECTION_CHANGE,
                                                   collectionChangeHandler,
                                                   false,
            ...
            else if (_rootModel is IHierarchicalData)
            {
                ...
                hierarchicalCollectionView.addEventListener(CollectionEvent.COLLECTION_CHANGE,
                                                   collectionChangeHandler,
                                                   false,
                                                   EventPriority.DEFAULT_HANDLER, true);
            }
        ...
        // because in no columns array has been defined, columns are generated
    ...
    private function addItemsToTarget(event:DragEvent, items:Array):void
    {
      ...
      if(!(obj is AdvancedDataGrid))
        ...
        else if (targetADG.collection.length == 0)
        {
            ...
            var _obj:Object = items.shift();
            targetADG.addChildItem(targetADG._dropData.parent,
                                   _obj, targetADG._dropData.index);
        }
    ...
    private function validateGroupedColumns(groupedColumns:Array):Boolean
    {
        ...
        for( k = 0; k < groupedColumns.length; k++)
            queue.push(groupedColumns[k]);
        ...
        while(queue.length > 0 && valid)
        {
            ...
            else
            {
                ...
                if(c is AdvancedDataGridColumnGroup)
                {
                    ...
                    if(cGroup.children)
                        for( k = 0; k < cGroup.children.length; k++)
                            queue.push(cGroup.children[k]);
                    }
            }
        }
    ...
    private function updateDropData(event:DragEvent):void
    {
        ...
        var topItem:Object = (rowNum > _verticalScrollPosition && rowNum < numItems) ? listItems[rowNum - _verticalScrollPosition - 1][0].data : null;
    ...
    override protected function headerNavigationHandler(event:KeyboardEvent):void
    {
       ...
       else
       {
          ...
          //RSE don't we allow vertical scrolling when focus is on the header?
          // if the focus is on header and we whit page-down/page-up no action would be taken
          // is this fine?
    ...
    {
        // Copied from Tree.keyDownHandler()
        var evt:ListEvent;
        var pt:Point;
    ...
    {
        // To Do - Check for the item renderer instead of the column number
        ...
        if (isCellSelectionMode())
        {
            ...
            for (var i:int = 0; i < n; i++)
            {
                ...
                if (cellSelectionData[uid])
                    for (var j:String in cellSelectionData[uid])
                    {
                        removeCellIndicators(uid, int(j));
                        removeCellSelectionData(uid, int(j));
                               ...
                               cause:Event = null):void
    {
        ...
        if (animate && rowIndex < rC && rowsToMove > 0 && rowsToMove < 20 && dur!=0)
        {
            ...
            if (renderer is IDropInListItemRenderer)
            {
                ...
                advancedDataGridListData = AdvancedDataGridListData(makeListData(advancedDataGridListData.item,
                                                                                 advancedDataGridListData.uid, advancedDataGridListData.rowIndex,col,visibleColumns[col]));
            ...
            else // closing up rows
            {
                ...
                for (i = 0; i < rowsToMove && yy < maxDist; i++)
                {
                    ...
                    else
                    {
                        ...
                        // End: Dont know why this is done
                        hh = rowHeight;
                    }
                    tmpRowInfo = new ListRowInfo(yy, hh, uid, data);
    ...
    mx_internal function onTweenUpdate(value:Object):void
    {
        ...
        for (i = rowIndex; i < n; i++)
        {
            ...
            if (listItems[i].length)
            {
                for (var col:int =0; col < listItems[i].length; ++col)
                {
                    renderer = IFlexDisplayObject(listItems[i][col]);
                    lastY = renderer.y;
        ...
        for (i = 0; i < n; i++)
        {
            ...
            {
                s = null;
                renderer = IFlexDisplayObject(rowList[i][col].item);
                ...
                lastY = renderer.y;
                renderer.move(renderer.x, rowList[i][col].itemOldY + value);
                deltaY = renderer.y - lastY;
    ...
    mx_internal function onTweenEnd(value:Object):void
    {
        ...
        if (opening)
        {
            ...
            for (i = rowIndex; i < rC; i++)
            {
                if (listItems[i].length)
                {
                    for (col =0; col < listItems[i].length; ++col)
                    {
                        renderer = listItems[i][col];
        ...
        else //closing
        {
            for (i = 0; i < rowList.length; i++)
            {
                for (col = 0; col < rowList[i].length; col++)
                {
                    mask = rowList[i][col].item.mask;
                    if (mask)
                    {
                        listContent.removeChild(mask);
                        rowList[i][col].item.mask = null;
                    }
                    addToFreeItemRenderers(rowList[i][col].item);
                    //kill graphic and graphic mask if necessary
                    itemUID = itemToUID(rowList[i][col].item.data);
                    ...
                    delete rowMap[rowList[i][col].item.name];
                }
            }
            for (i = rowIndex; i < rC; i++)
            {
    ...
    private function buildUpCollectionEvents(open:Boolean):Array
    {
        ...
        if (open)
        {
            ...
            // Integrated from Changelist 160845
            // M1 bugfix 197066 - Calling expandChildrenOf() generates RTE if one of the children is an empty branch.
            if (!children)
                return [];
            // End Integrated from Changelist 160845
            var cursor:IViewCursor = children.createCursor();
            ...
            while (!cursor.afterLast)
            {
                ...
                {
                    // will not come here - just to be safe
                    break;
                }
            }
        }
            ...
            if (childItems)
            {
                ...
                while (childItem)
                {
                    if (isBranch(childItem))
                        expandChildrenOf(childItem, open);
    ...
    private function setDesignViewData():void
    {
        ...
        {
            var branchName:String = columns[0].dataField || "";
    ...
    private function setDesignViewSort():void
    {
      if (collection == null)
        ...
        for (var colNum:int = 0; colNum < columns.length; colNum++)
        {
            sort.fields.push(
                    new SortField(columns[colNum].dataField,
                                  false,
                                  oddDescending,
                                  false)
                    );
    ...
    {
        if (_rootModel && _rootModel is IGroupingCollection)
        {
            // group header
    ...
    public function expandAll():void
    {
        if (dataProvider is IHierarchicalCollectionView && iterator)
        {
            ...
            {
                expandChildrenOf(iterator.current, true);
                iterator.moveNext();
            }
    ...
    override protected function isHeaderItemRenderer(item:IListItemRenderer):Boolean
    {
        ...
        for (var i:int = 0; i < n; i++)
        {
            if (UIDUtil.getUID(orderedHeadersList[i].headerItem) == UIDUtil.getUID(item))
                return true;
        }
        return false;
    }
    ...
    protected function cellNavigationHandler(event:KeyboardEvent):void
    {
        ...
        for (var i:int = 0; i < validKeys.length; i++)
        {
        ...
        if (!isValidKey)
        {
            // Copied from ListBase::keyDownHandler().
            ...
            case Keyboard.F2:
            {
                if (caretColumnIndex != -1)
                {
                    ...
                    if (newData && isDataEditable(newData))
                    {
                        ...
                        // copied from finishCellKeySelection()
                        dispatchEvent(advancedDataGridEvent);
                    }
                }
                break;
    ...
    {
        if (!sortExpertMode)
            if (visualSortInfo[column.colNum])
    ...
    */
                                                ...
                                                transition:Boolean = false):void
    {
        if (isCellSelectionMode())
        {
            if (visibleCellRenderers[uid])
            {
                for (var columnIndex:int = 0; columnIndex < _columns.length; columnIndex++)
                {
    ...
    */
                                      ...
                                      transition:Boolean = true):Boolean
    {
        ...
        else if (ctrlKey && selectionMode == MULTIPLE_CELLS)
        {
            ...
            else
            {
                ...
                selectedColumnIndex = columnIndex;
                // XXX Setting this causes unexpected behavior in the UI because
     ...
     */
    private function isCellAlreadySelected(givenCell:Object):Boolean
    {
        ...
        for (var i:int = 0; i < n; i++)
            if (selectedCells[i].rowIndex == givenCell.rowIndex
                    && selectedCells[i].columnIndex == givenCell.columnIndex)
                return true;
    ...
    mx_internal function addToSelectedCells(rowIndex:int, columnIndex:int):void
    {
        ...
        if (isCellAlreadySelected(newCell))
            return;
    ...
    {
        for (var uniqueID:String in cellSelectionData)
        {
            for (var columnIndexString:String in cellSelectionData[uniqueID])
            {
                var data:Object = cellSelectionData[uniqueID][columnIndexString].data;

                removeCellSelectionData(uniqueID, int(columnIndexString));

                var item:IListItemRenderer;
                if (visibleCellRenderers[itemToUID(data)])
                    item = visibleCellRenderers[itemToUID(data)][columnIndexString];
                if (item)
                    drawCellItem(item, false,
                                 uniqueID == highlightUID
                                 && highlightColumnIndex == int(columnIndexString),
                                 false,
                                 transition);
            }
        }
...
//             itemXPos = getAdjustedXPos(itemXPos);
    ...
    override protected function removeIndicators(uid:String):void
    {
        if (isCellSelectionMode())
        {
            ...
            for (columnIndexString in cellSelectionIndicators[uid])
            {
                removeCellIndicators(uid, int(columnIndexString));
            }

            for (columnIndexString in visibleCellRenderers[uid])
            {
                delete visibleCellRenderers[uid][columnIndexString];
            }
    ...
    {
        for (var uid:String in cellSelectionIndicators)
        {
            for (var columnIndexString:String in cellSelectionIndicators[uid])
            {
                removeCellIndicators(uid, int(columnIndexString));
            }
        }
    ...
    override protected function moveIndicators(uid:String, offset:int, absolute:Boolean):void
    {
         ...
         if (isCellSelectionMode())
        {
            if (cellSelectionIndicators[uid])
            {
                ...
                {
                    if (absolute)
                        cellSelectionIndicators[uid][columnIndexString].y = offset;
                    else
                        cellSelectionIndicators[uid][columnIndexString].y += offset;
                }
            }
    ...
    mx_internal function setSelectedCells(value:Array):void
    {
        ...
        for (var i:int = 0; i < n; i++)
        {
            if (isCellAlreadySelected(value[i]))
                continue;
